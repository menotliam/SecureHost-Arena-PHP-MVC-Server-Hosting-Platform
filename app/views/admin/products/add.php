        ...
        <section class="card panel-card">
            <div class="card-body">
                <div class="panel-header flex-wrap gap-2 mb-3">
                    <h2 class="panel-title mb-0">Thêm sản phẩm / gói server</h2>
                ...
                <form action="<?php echo URLROOT; ?>/admin/products/add" method="POST" enctype="multipart/form-data">
                    <div class="row g-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="category_id" id="category_id" data-admin-custom-select="true">
                                    <?php foreach ($data['categories'] as $category): ?>
                                        <option value="<?php echo (int) $category->id; ?>" <?php echo (isset($data['category_id']) && (int) $data['category_id'] === (int) $category->id) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) $category->name, ENT_QUOTES, 'UTF-8'); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control <?php echo !empty($data['image_err']) ? 'is-invalid' : ''; ?>" id="image" name="image" required accept=".jpg,.jpeg,.png,.gif,.webp,image/*">
                                <p class="form-text panel-muted mb-0">Chấp nhận: jpg, jpeg, png, gif, webp.</p>
                    ...
                    <div class="text-end">
                        <button type="submit" class="btn btn-primary"><i class="ti-save"></i> Lưu thông tin</button>
                    </div>
                </form>
            </div>
        </section>
